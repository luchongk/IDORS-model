...
class SVM:
    ...
    def fit(self, word_vectors, sent_vectors, labels):
        final_vectors = np.zeros((len(sent_vectors), 1600))
    ...
    def evaluate(self, word_vectors, sent_vectors, labels):
        final_vectors = np.zeros((len(sent_vectors), 1600))
        for index, v in enumerate(word_vectors):
            list_of_modes = self._get_modes_from_word_vecs(v)
            list_of_modes.append(sent_vectors[index])
